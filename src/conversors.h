...
long long dec2bin(int num) {
    ...
    while (num != 0) {
        ...
        ret += rem * f;    // Add it to the next digit of the return (f is the
                           // position)
...
int bin2dec(long long bin) {
    ...
    int p   = 0;    // Power that will be used when increasing ret.
...
int dec2oct(int num) {
    return bin2oct(dec2bin(num));    // We need to revert it because of the
                                     // order we are converting binary
}
...
char* format_oct(int oct) {
    ...
    // Final string. Max digits * chars per digit
    char* ret  = calloc(max_digits * charsperdigit, sizeof(char));
    int strpos = max_digits * charsperdigit - 2;    // -1 cuz of the len and -2 cuz
                                                    // of the ret +1
...
int bin2oct(long long num) {
    ...
    while (num > 0) {
        ...
        ret += bin2dec(num % 1000);    // Convert the last 3 digits of the num to
                                       // decimal
...
char* bin2hex(long long num) {
    ...
    char* ret = calloc(max_chars, sizeof(char));    // Hexadecimal result that will
                                                    // be returned

    int strpos = max_chars - 2;
    while (num > 0 && strpos >= 0) {
        // Convert the last 3 digits of the bin to decimal and then to hex char
        ret[strpos--] += int2hex(bin2dec(num % 10000));

        // Remove the last 3 digits from bin
        num /= 10000;
    }
...
long long oct2bin(int num) {
    long long ret = 0;
    long cur_bin  = 0;    // Current digit of the octal converted to binary.
                          // Needed to store in case we need to add left 0's
    ...
    while (num > 0) {
         ...
         */
        for (int n = 0; n < digits; n++) cur_bin *= 10;
...
long long hex2bin(const char* str) {
    ...
    while ((c = str[strpos++]) != '\0') {
        ...
        // Convert the current pos of the string from left to right to integer and
        // then to binary, then add it to the binary long that will be returned
